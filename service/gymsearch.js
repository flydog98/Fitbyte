...
const jsonContent = fs.readFileSync("./config/key.json", "utf8");
...
async function getExercise(locationName) {
  var mergedResult;
	const mntResult = {};
	searchGym(locationName)
		.then((resultJson) => {
			Object.assign(mntResult, resultJson);
		})
		.catch((error) => {
			console.error(error);
		});
	...
	s3.getObject(
		...
		(err, data) => {
			...
			lines.forEach((line) => {
				// 쉼표를 기준으로 위치와 이름을 분리
				const [locationName, name] = line.split(",");
				//if(locationName ==NULL|| name ==NULL){

				//}
				//else{
				// 위치가 이미 gyms 객체에 저장되어 있다면 이름을 추가, 없다면 새로운 위치를 추가하고 이름을 저장
				if (gyms[locationName]) {
					gyms[locationName].push(name);
				} else {
					gyms[locationName] = [name];
				}
				//}
			});

			// 입력한 위치와 일치하는 이름들을 gyms 객체에서 찾아서 배열로 반환
			const gymNames = gyms[locationName] || [];
			...
			// 결과를 JSON 형식으로 반환
			const csvResult = { locationName, gyms: limitedGymNames };
			// Create a merged result object
			mergedResult = {
				locationame: mntResult.locationname,
				mntn_nm: mntResult.mntn_nm,
				gyms: csvResult.gyms,
			};
		}
	)
